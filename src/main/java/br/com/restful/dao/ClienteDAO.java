...
public class ClienteDAO extends ConnectionFactory {
	 ...
	 * @return verdade se atualizado e falso se nao.
	 */
	public boolean updatePATCH(long id, Cliente cliente) {
		boolean isAtualizado = false;
		PreparedStatement pstmt = null;
		Connection conexao = criarConexao();
		try {
			if(cliente.getNome().isEmpty() && cliente.getCpf().isEmpty()){
			pstmt = conexao.prepareStatement("UPDATE cliente SET endereco = ? WHERE id = ?");
			pstmt.setString(1, cliente.getEndereco());
			pstmt.setLong(2, id);
			int execute = pstmt.executeUpdate();
			isAtualizado = true;
			System.out.println("Retorno update: " + execute);
			}else if (cliente.getEndereco().isEmpty() && cliente.getCpf().isEmpty()){
				pstmt = conexao.prepareStatement("UPDATE cliente SET nome = ? WHERE id = ?");
				pstmt.setString(1, cliente.getNome());
				pstmt.setLong(2, id);
				int execute = pstmt.executeUpdate();
				isAtualizado = true;
				System.out.println("Retorno update: " + execute);
			}else if (cliente.getEndereco().isEmpty() && cliente.getNome().isEmpty()){
				pstmt = conexao.prepareStatement("UPDATE cliente SET cpf = ? WHERE id = ?");
